...
```bash
git clone https://github.com/anza-xyz/agave.git --depth 1 --branch v3.0.6 ~/agave-src-v3.0.6
```

Then I run the `cargo-install-all.sh` script to build and install Agave. I assign `~/.local/share/solana/install/releases/3.0.6` as my installation directory, to replicate the directory structure of the pre-built binaries.
```bash
bash ~/agave-src-v3.0.6/scripts/cargo-install-all.sh ~/.local/share/solana/install/releases/3.0.6
```
...
When installing the *pre-built* binaries, the installer automatically creates a symbolic link from the installation directory (`*/3.0.6`) to another directory (`*/active_release`). This link can be used in service files and environment variables, so these won't have to be amended on every Agave update. When building from source I have to create that link manually:
```bash
ln --symbolic ~/.local/share/solana/install/releases/3.0.6 ~/.local/share/solana/install/active_release
```
...
ln --force --no-dereference --symbolic \
  ~/.local/share/solana/install/releases/3.0.6 ~/.local/share/solana/install/active_release
```
