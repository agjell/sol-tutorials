...
The tutorial has been tested on a fresh install of Ubuntu 26.04.1 LTS. Please post an issue if something is not working.
...
```bash
git clone https://github.com/anza-xyz/agave.git --depth 1 --branch v4.2.2 ~/agave-src-v4.2.2
```

Then I run the `cargo-install-all.sh` script to build and install Agave. I assign `~/.local/share/solana/install/releases/4.2.2` as my installation directory, to replicate the directory structure of the pre-built binaries.

```bash
bash ~/agave-src-v4.2.2/scripts/cargo-install-all.sh ~/.local/share/solana/install/releases/4.2.2
```
...
When installing the *pre-built* binaries, the installer automatically creates a symbolic link from the installation directory (`*/4.2.2`) to another directory (`*/active_release`). This link can be used in service files and environment variables, so these won't have to be amended on every Agave update. When building from source I have to create that link manually:

```bash
ln --symbolic ~/.local/share/solana/install/releases/4.2.2 ~/.local/share/solana/install/active_release
```
...
ln --force --no-dereference --symbolic \
  ~/.local/share/solana/install/releases/4.2.2 ~/.local/share/solana/install/active_release
```
